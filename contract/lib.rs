...
mod meme_vote {
    ...
    impl MemeVote {
        ...
        pub fn get_top_memes(&self, limit: u32) -> Vec<Meme> {
            ...
            // Сортируем по количеству лайков (убывание)
            // Простая сортировка пузырьком для демо (O(n²))
            // В продакшене лучше использовать более эффективный алгоритм
            for i in 0..memes.len() {
                for j in 0..memes.len() - i - 1 {
                    if memes[j].likes < memes[j + 1].likes {
                        memes.swap(j, j + 1);
                    }
                }
            }
